...
This project is a project which helps me to pratice globally JUnit5 Jupiter. The aim of this project is to use JUnit5 in different cases. In this project, I will develop some small classes like Number with some functions.
...
To use JUnit5 Jupiter, you must use a Java version **equals or geater** than 8.

---

## 1.3. Dependencies

In the next dependencies, you will note that I restricted its scope to the test one. That is because JUnit will just be used in the test one. So, we do not need to keep the dependency on the production scope.

I use two JUnit dependencies:

### 1.3.1. JUnit Jupiter Platform

JUnit Jupiter Platform, which is used thanks the junit-jupiter-engine dependency, allows tests running. It give the TestEngine API too. This API is used to create new test frameworks. These frameworks can be imported into the app and run by JUnit Jupiter Platform.
...
Jupiter is the 5*th* version of JUnit. But, you can "mix" the 3*rd* and 4*th* versions with the 5*th*. Jupiter Vintage imports olds versions methods. That is usefull if you upgrade your JUnit version parts by parts.

---

## 1.4. Build plugins

The following build plugins allows us to launch tests from maven. We need them because surfire does not support JUnit5 natively.
...
There is a lot of assertions methods, you can find them in the class `org.junit.jupiter.api.Assertions`. It can be usefull to import these methods with a static import like this: `import static org.junit.jupiter.api.Assertions.*;`  
In most of the following methods can take, optionnally, a last argument. This argument is a message which will be prompted if the assertion fails.
...
BeforeAll must be placed on a method. The marked method will be run once before all other tests method in the current class. It is a usefull annotation to prepare a global context for your tests.
...
As BeforeAll, this annotation will mark a method as being executed before tests in the current class. But here, the BeforeEach method will be run before each new test. It can be usefull to reinitialize a context before a new test.
...
The AfterAll annotation is the opposite from the BeforeAll one. It will be run after all tests in the current class. It is used mostly to clean up your environnement or your test database.
...
The AfterEach annotation is the opposite from the BeforeEach one. It will be run after each test in the current class. It is used mostly to clean up your environnement or your test database after one test or to reinitialize it.
